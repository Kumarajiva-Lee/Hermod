...
contains
  ...
  subroutine log_warning(message, file, line, pid)
  ...
  subroutine log_error(message, file, line, pid)
    ...
    logical mute
    is_redirected = .not. isatty(6)
    mute = merge(pid /= 0, .false., present(pid))
  ...
  subroutine log_print_diag(prefix, pid)
    ...
    if (merge(pid /= 0, .false., present(pid))) return
    ...
    do while (.not. iter%ended())
      ...
      type is (real(4))
        write(6, '(X, A)', advance='no') trim(to_str(value, 'ES25.15'))
      type is (real(8))
        write(6, '(X, A)', advance='no') trim(to_str(value, 'ES25.15'))
      class default
