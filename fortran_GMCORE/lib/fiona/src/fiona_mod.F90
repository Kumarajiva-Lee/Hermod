module fiona_mod
  ...
  interface fiona_output
    ...
    module procedure fiona_output_6d
    module procedure fiona_output_7d
  ...
  interface fiona_input
    ...
    module procedure fiona_input_6d
    module procedure fiona_input_7d
...
contains
  ...
  subroutine fiona_final()

    call datasets%clear()

  end subroutine fiona_final
  ...
  subroutine fiona_create_dataset(dataset_name, desc, file_prefix, file_path, start_time, time_units, mpi_comm, group_size, split_file)
    ...
    if (datasets%hashed(dataset_name)) then
      ...
      dataset%desc = desc_
      call dataset%atts%init()
      call dataset%dims%init()
      call dataset%vars%init()
  ...
  subroutine fiona_add_dim(dataset_name, name, long_name, units, size, add_var, decomp)
    ...
    if (present(add_var)) then
      if (add_var) then
        ...
        call fiona_add_var(dataset_name, name, long_name=dim%long_name, units=dim%units, dim_names=[name], dtype='real(8)')
      end if
    end if
  ...
  subroutine fiona_add_var(dataset_name, name, long_name, units, dim_names, dtype, missing_value)
    ...
    if (.not. present(dtype)) then
      ...
      case (4)
        var%dtype = NF90_FLOAT
      case (8)
        var%dtype = NF90_DOUBLE
      end select
    else
      select case (dtype)
      case ('real', 'real(4)', 'r4')
      ...
      case ('real(8)', 'r8')
        var%dtype = NF90_DOUBLE
      case ('integer(2)', 'i2')
        var%dtype = NF90_SHORT
      case ('integer', 'integer(4)', 'i4')
        var%dtype = NF90_INT
      case ('integer(8)', 'i8')
        var%dtype = NF90_INT64
    ...
    if (present(missing_value)) then
      ...
      type is (integer(2))
        allocate(var%i2_missing_value)
        var%i2_missing_value = missing_value
      type is (integer(4))
  ...
  subroutine fiona_start_output(dataset_name, time_in_seconds, new_file, tag)
...
#ifdef HAS_MPI
    if (dataset%mpi_comm /= MPI_COMM_NULL .and. .not. dataset%enable_parallel .and. dataset%proc_id > 0) then
    ...
    if (associated(dataset%time_var)) then
      ...
      if (.not. new_file .or. dataset%time_step == 0) then
        ierr = NF90_INQUIRE_DIMENSION(dataset%id, dataset%time_var%id, len=dataset%time_step)
      end if
      if (time_in_seconds /= dataset%time_in_seconds .or. dataset%enable_parallel) then
        if (dataset%proc_id == 0 .or. dataset%enable_parallel) dataset%time_step = dataset%time_step + 1
...
#ifdef HAS_MPI
        if (dataset%mpi_comm /= MPI_COMM_NULL .and. dataset%enable_parallel) then
  ...
  subroutine apply_dataset_to_netcdf_master(file_path, dataset, new_file)
    ...
    if (present(new_file)) then
      if (new_file) then
        ...
        if (ierr == -114) then
#ifdef HAS_MPI
          dataset%enable_parallel = .false.
          if (dataset%proc_id == 0) then
            call log_warning('Disable parallel IO due to netCDF library is not built with --enable-parallel!')
          end if
#endif
        ...
        if (dataset%mpi_comm /= MPI_COMM_NULL .and. dataset%enable_parallel) then
          ...
          if (dataset%mpi_comm /= MPI_COMM_NULL .and. dataset%proc_id > 0) return
  ...
  subroutine apply_dataset_to_netcdf_slave(file_path, dataset, new_file)
    ...
    ierr = NF90_OPEN(file_path, NF90_WRITE, dataset%id)
    ...
    it = hash_table_iterator(dataset%vars)
    do while (.not. it%ended())
      var => dataset%get_var(it%key)
      ierr = NF90_INQ_VARID(dataset%id, var%name, var%id)
  ...
  subroutine fiona_output_6d(dataset_name, name, array, start, count)
  ...
  end subroutine fiona_output_6d
  
  subroutine fiona_output_7d(dataset_name, name, array, start, count)
  ...
  end subroutine fiona_output_7d
...
#ifdef HAS_MPI
    if (dataset%mpi_comm /= MPI_COMM_NULL) then
      if (dataset%enable_parallel) then
        call MPI_BARRIER(dataset%mpi_comm, ierr)
      else if (dataset%proc_id + 1 < dataset%num_proc) then
  ...
  subroutine fiona_input_6d(dataset_name, var_name, array, start, count, time_step)

    character(*), intent(in ) :: dataset_name
    character(*), intent(in ) :: var_name
    class    (*), intent(out) :: array(:,:,:,:,:,:)
    integer     , intent(in ), optional :: start(6)
    integer     , intent(in ), optional :: count(6)
    integer     , intent(in ), optional :: time_step
    ...
    type is (integer(2))
      if (present(time_step)) then
        if (present(start) .and. present(count)) then
          ierr = NF90_GET_VAR(dataset%id, varid, array,                              &
            start=[start(1),start(2),start(3),start(4),start(5),start(6),time_step], &
            count=[count(1),count(2),count(3),count(4),count(5),count(6),        1])
        else
          ierr = NF90_GET_VAR(dataset%id, varid, array,                                                                  &
            start=[             1,             1,             1,             1,             1,             1,time_step], &
            count=[size(array, 1),size(array, 2),size(array, 3),size(array, 4),size(array, 5),size(array, 6),        1])
        end if
      else
        if (present(start) .and. present(count)) then
          ierr = NF90_GET_VAR(dataset%id, varid, array,                    &
  ...
  end subroutine fiona_input_6d
  
  subroutine fiona_input_7d(dataset_name, var_name, array, start, count, time_step)

    character(*), intent(in ) :: dataset_name
    character(*), intent(in ) :: var_name
    class    (*), intent(out) :: array(:,:,:,:,:,:,:)
    integer     , intent(in ), optional :: start(7)
    integer     , intent(in ), optional :: count(7)
    integer     , intent(in ), optional :: time_step
    ...
    type is (integer(2))
      if (present(time_step)) then
        if (present(start) .and. present(count)) then
          ierr = NF90_GET_VAR(dataset%id, varid, array,                                       &
            start=[start(1),start(2),start(3),start(4),start(5),start(6),start(7),time_step], &
            count=[count(1),count(2),count(3),count(4),count(5),count(6),count(7),        1])
        else
          ierr = NF90_GET_VAR(dataset%id, varid, array,                                                                                 &
            start=[             1,             1,             1,             1,             1,             1,             1,time_step], &
            count=[size(array, 1),size(array, 2),size(array, 3),size(array, 4),size(array, 5),size(array, 6),size(array, 7),        1])
        end if
      else
        if (present(start) .and. present(count)) then
          ierr = NF90_GET_VAR(dataset%id, varid, array,                             &
