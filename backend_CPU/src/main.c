...
void LatLonMeshInit(struct HybridMeshField* mesh)
{
  ...
  t = 0;
  loopinfo = (struct MeshVector6){1,1,360+2*Proc.lon_hw,0,0,-Proc.lon_hw + 1};
  dlon = (double)(((6.283185307179586 - 0) / 360));
  for(i=0; i<360+2*Proc.lon_hw; i+=1){
  ...
  dlat0 = (double)(((1.5707963267948966 - -1.5707963267948966) / 179));
  t = 0;
  loopinfo = (struct MeshVector6){1,179,1,0,1,0};
  for(j=0+Proc.lat_hw; j<179+Proc.lat_hw; j+=1){
    v_inx = disect(t,&loopinfo);
    t = (t + 1);
  ...
  for(j=1+Proc.lat_hw; j<179+Proc.lat_hw; j+=1){
  ...
  for(j=179+Proc.lat_hw; j<180+Proc.lat_hw; j+=1){
    mesh->full_lat[0][j][0] = 1.5707963267948966;
    mesh->full_lat_deg[0][j][0] = (1.5707963267948966 * 57.29577951308232);
  }
  ...
  for(j=0; j<180+2*Proc.lat_hw; j+=1){
  ...
  for(j=0+Proc.lat_hw; j<178+Proc.lat_hw; j+=1){
    for(i=0+Proc.lon_hw; i<1+Proc.lon_hw; i+=1){
      buf = cartesian_transform(mesh->full_lon[0][0][(i + 1)],mesh->full_lat[0][(j + 1)][0]);
      v1.x = buf.x;
      v2.x = buf.y;
  ...
  for(j=178+Proc.lat_hw; j<179+Proc.lat_hw; j+=1){
  ...
  for(j=0+Proc.lat_hw; j<180+Proc.lat_hw; j+=1){
...
{
  int nid;
  nid = ncOpenFile("gmcore.restart.DSL.nc",0);
  ...
  UpdateHalo_3d_D(Proc, &state->u_lon[0][0][0], &Proc.FieldReq[async_u_lon], false, true, true, true, true, true, true);
  ...
  UpdateHalo_3d_D(Proc, &state->v_lat[0][0][0], &Proc.FieldReq[async_v_lat], true, false, true, true, true, true, true);
  ...
  UpdateHalo_3d_D(Proc, &state->pt[0][0][0], &Proc.FieldReq[async_pt], true, true, true, true, true, true, true);
  
  ncGetVar(nid, "phs", 0, 1, 0, Proc.lat_beg, Proc.full_nlat, Proc.lat_hw, Proc.lon_beg, Proc.full_nlon, Proc.lon_hw, &(state->phs[0][0][0]));
  UpdateHalo_2d_D(Proc, &state->phs[0][0][0], &Proc.FieldReq[async_phs], true, true, true, true, true, true);
  
  ncGetVar(nid, "gzs", 0, 1, 0, Proc.lat_beg, Proc.full_nlat, Proc.lat_hw, Proc.lon_beg, Proc.full_nlon, Proc.lon_hw, &(staticv->gzs[0][0][0]));
  UpdateHalo_2d_D(Proc, &staticv->gzs[0][0][0], &Proc.FieldReq[async_gzs], true, true, true, true, true, true);
...
void spaceOperatorInit_0(struct HybridStaticField* staticv, struct HybridMeshField* mesh)
{
  ...
  for(j=MAX(Proc.lat_beg, 1)-Proc.lat_beg+Proc.lat_hw; j<MIN(Proc.lat_end+1, 179)-Proc.lat_beg+Proc.lat_hw; j+=1){
    for(i=MAX(Proc.lon_beg, 0)-Proc.lon_beg+Proc.lon_hw; i<MIN(Proc.lon_end+1, 360)-Proc.lon_beg+Proc.lon_hw; i+=1){
      staticv->dzsdlon[0][j][i] = (((staticv->gzs[0][j][(i + 1)] - staticv->gzs[0][j][i]) / 9.80616) / mesh->de_lon[0][j][0]);
    }
  }
  
  HaloWait(Proc,&Proc.FieldReq[async_gzs]);
  for(j=MAX(Proc.lat_beg, 0)-Proc.lat_beg+Proc.lat_hw; j<MIN(Proc.lat_end+1, 179)-Proc.lat_beg+Proc.lat_hw; j+=1){
    for(i=MAX(Proc.lon_beg, 0)-Proc.lon_beg+Proc.lon_hw; i<MIN(Proc.lon_end+1, 360)-Proc.lon_beg+Proc.lon_hw; i+=1){
...
void gzlevInit_0(struct HybridStateField* state, struct HybridStaticField* staticv)
{
  ...
  HaloWait(Proc,&Proc.FieldReq[async_gzs]);
  for(k=32+Proc.lev_hw; k<33+Proc.lev_hw; k+=1){
    for(j=MAX(Proc.lat_beg, 0)-Proc.lat_beg+Proc.lat_hw; j<MIN(Proc.lat_end+1, 180)-Proc.lat_beg+Proc.lat_hw; j+=1){
      for(i=MAX(Proc.lon_beg, 0)-Proc.lon_beg+Proc.lon_hw; i<MIN(Proc.lon_end+1, 360)-Proc.lon_beg+Proc.lon_hw; i+=1){
        state->gz_lev[k][j][i] = staticv->gzs[0][j][i];
      }
    }
  }
  UpdateHalo_2d_D(Proc, &state->gz_lev[32 + Proc.lev_hw][0][0], &Proc.FieldReq[async_gz_lev], true, true, false, true, false, true);
...
void uvInit_0(struct HybridStateField* state)
{
  ...
  for(k=0+Proc.lev_hw; k<32+Proc.lev_hw; k+=1){
    for(j=MAX(Proc.lat_beg, 1)-Proc.lat_beg+Proc.lat_hw; j<MIN(Proc.lat_end+1, 179)-Proc.lat_beg+Proc.lat_hw; j+=1){
      for(i=MAX(Proc.lon_beg, 0)-Proc.lon_beg+Proc.lon_hw; i<MIN(Proc.lon_end+1, 359)-Proc.lon_beg+Proc.lon_hw; i+=1){
...
void operatorPrepareNull_0(struct HybridStateField* state, struct HybridStaticField* staticv, struct HybridAdvField* advpt, struct HybridAdvPara* advPara, struct HybridMeshField* mesh, double dt)
{
  ...
  UpdateHalo_3d_D(Proc, &state->ph_exn_lev[0][0][0], &Proc.FieldReq[async_ph_exn_lev], true, true, false, false, true, false, true);
  ...
  UpdateHalo_3d_D(Proc, &state->m[0][0][0], &Proc.FieldReq[async_m], true, true, true, false, true, false, true);
  ...
  if (((advPara->uv_step) == -1))
  {
    ...
    UpdateHalo_3d_D(Proc, &advpt->uu[0][0][0], &Proc.FieldReq[async_uu], false, true, true, true, false, false, false);
    ...
    UpdateHalo_3d_D(Proc, &advpt->vv[0][0][0], &Proc.FieldReq[async_vv], true, false, true, false, false, true, false);
  ...
  if (((advPara->dynamic) || ((advPara->uv_step) > (advPara->nstep))))
  {
    ...
    // SumCall:state->tmpsum.[0, 0, 32, 0, 0, 1, 1, 0, 360]
    ...
    for(k=0+Proc.lev_hw; k<32+Proc.lev_hw; k+=1){
      for(j=MAX(Proc.lat_beg, 0)-Proc.lat_beg+Proc.lat_hw; j<MIN(Proc.lat_end+1, 1)-Proc.lat_beg+Proc.lat_hw; j+=1){
        for(i=MAX(Proc.lon_beg, 0)-Proc.lon_beg+Proc.lon_hw; i<MIN(Proc.lon_end+1, 360)-Proc.lon_beg+Proc.lon_hw; i+=1){
          advpt->divy[k][j][i] = (((state->tmpsum[k][j][i] * mesh->le_lat[0][j][0]) / 360) / mesh->area_cell[0][j][0]);
        }
      }
    }
  ...
  UpdateHalo_3d_D(Proc, &state->mfx_lon[0][0][0], &Proc.FieldReq[async_mfx_lon], false, true, true, true, false, false, true);
  ...
  UpdateHalo_3d_D(Proc, &state->mfy_lat[0][0][0], &Proc.FieldReq[async_mfy_lat], true, false, true, false, true, true, false);
  ...
  for(k=0+Proc.lev_hw; k<32+Proc.lev_hw; k+=1){
    for(j=MAX(Proc.lat_beg, 0)-Proc.lat_beg+Proc.lat_hw; j<MIN(Proc.lat_end+1, 1)-Proc.lat_beg+Proc.lat_hw; j+=1){
      for(i=MAX(Proc.lon_beg, 0)-Proc.lon_beg+Proc.lon_hw; i<MIN(Proc.lon_end+1, 360)-Proc.lon_beg+Proc.lon_hw; i+=1){
        state->ke[k][j][i] = (state->tmpsum[k][j][i] / 360);
      }
    }
  }
  UpdateHalo_3d_D(Proc, &state->ke[0][0][0], &Proc.FieldReq[async_ke], true, true, true, false, true, false, true);
  ...
  for(k=0+Proc.lev_hw; k<32+Proc.lev_hw; k+=1){
    for(j=MAX(Proc.lat_beg, 0)-Proc.lat_beg+Proc.lat_hw; j<MIN(Proc.lat_end+1, 1)-Proc.lat_beg+Proc.lat_hw; j+=1){
      for(i=MAX(Proc.lon_beg, 0)-Proc.lon_beg+Proc.lon_hw; i<MIN(Proc.lon_end+1, 360)-Proc.lon_beg+Proc.lon_hw; i+=1){
        state->vor[k][j][i] = ((state->tmpsum[k][j][i] / 360) / mesh->area_cell[0][j][0]);
      }
    }
  }
  ...
  UpdateHalo_3d_D(Proc, &state->pv[0][0][0], &Proc.FieldReq[async_pv], false, false, true, true, true, true, true);
  ...
  UpdateHalo_3d_D(Proc, &state->pv_lon[0][0][0], &Proc.FieldReq[async_pv_lon], false, true, true, true, false, false, true);
  ...
  UpdateHalo_3d_D(Proc, &state->pv_lat[0][0][0], &Proc.FieldReq[async_pv_lat], true, false, true, false, true, true, false);
  ...
  for(k=0+Proc.lev_hw; k<32+Proc.lev_hw; k+=1){
    for(j=MAX(Proc.lat_beg, 0)-Proc.lat_beg+Proc.lat_hw; j<MIN(Proc.lat_end+1, 1)-Proc.lat_beg+Proc.lat_hw; j+=1){
      for(i=MAX(Proc.lon_beg, 0)-Proc.lon_beg+Proc.lon_hw; i<MIN(Proc.lon_end+1, 360)-Proc.lon_beg+Proc.lon_hw; i+=1){
        state->div[k][j][i] = (((state->tmpsum[k][j][i] * mesh->le_lat[0][j][0]) / 360) / mesh->area_cell[0][j][0]);
      }
    }
  }
  UpdateHalo_3d_D(Proc, &state->div[0][0][0], &Proc.FieldReq[async_div], true, true, true, false, true, false, true);
  ...
  UpdateHalo_3d_D(Proc, &state->gz_lev[0][0][0], &Proc.FieldReq[async_gz_lev], true, true, false, false, true, false, true);
...
void stepForwardBackward_0(struct HybridStateField* old_state, struct HybridStateField* star_state, struct HybridStateField* new_state, struct HybridStaticField* staticv, struct HybridTendField* tend1, struct HybridTendField* tend2, struct HybridTendPara* tendPara, struct HybridAdvField* advpt, struct HybridAdvPara* advPara, struct HybridMeshField* mesh, double dt)
{
  ...
  if (((advPara->dynamic) || ((advPara->uv_step) > (advPara->nstep))))
  {
    ...
    // SumCall:star_state->tmpsum.[0, 0, 32, 0, 0, 1, 1, 0, 360]
    ...
    for(k=0+Proc.lev_hw; k<32+Proc.lev_hw; k+=1){
      for(j=MAX(Proc.lat_beg, 0)-Proc.lat_beg+Proc.lat_hw; j<MIN(Proc.lat_end+1, 1)-Proc.lat_beg+Proc.lat_hw; j+=1){
        for(i=MAX(Proc.lon_beg, 0)-Proc.lon_beg+Proc.lon_hw; i<MIN(Proc.lon_end+1, 360)-Proc.lon_beg+Proc.lon_hw; i+=1){
          advpt->divy[k][j][i] = (((star_state->tmpsum[k][j][i] * mesh->le_lat[0][j][0]) / 360) / mesh->area_cell[0][j][0]);
        }
      }
    }
  ...
  UpdateHalo_3d_D(Proc, &star_state->mfx_lon[0][0][0], &Proc.FieldReq[async_mfx_lon], false, true, true, true, false, false, true);
  ...
  UpdateHalo_3d_D(Proc, &star_state->mfy_lat[0][0][0], &Proc.FieldReq[async_mfy_lat], true, false, true, false, true, true, false);
  ...
  for(k=0+Proc.lev_hw; k<32+Proc.lev_hw; k+=1){
    for(j=MAX(Proc.lat_beg, 0)-Proc.lat_beg+Proc.lat_hw; j<MIN(Proc.lat_end+1, 1)-Proc.lat_beg+Proc.lat_hw; j+=1){
      for(i=MAX(Proc.lon_beg, 0)-Proc.lon_beg+Proc.lon_hw; i<MIN(Proc.lon_end+1, 360)-Proc.lon_beg+Proc.lon_hw; i+=1){
        star_state->ke[k][j][i] = (star_state->tmpsum[k][j][i] / 360);
      }
    }
  }
  UpdateHalo_3d_D(Proc, &star_state->ke[0][0][0], &Proc.FieldReq[async_ke], true, true, true, false, true, false, true);
  ...
  for(k=0+Proc.lev_hw; k<32+Proc.lev_hw; k+=1){
    for(j=MAX(Proc.lat_beg, 0)-Proc.lat_beg+Proc.lat_hw; j<MIN(Proc.lat_end+1, 1)-Proc.lat_beg+Proc.lat_hw; j+=1){
      for(i=MAX(Proc.lon_beg, 0)-Proc.lon_beg+Proc.lon_hw; i<MIN(Proc.lon_end+1, 360)-Proc.lon_beg+Proc.lon_hw; i+=1){
        star_state->div[k][j][i] = (((star_state->tmpsum[k][j][i] * mesh->le_lat[0][j][0]) / 360) / mesh->area_cell[0][j][0]);
      }
    }
  }
  UpdateHalo_3d_D(Proc, &star_state->div[0][0][0], &Proc.FieldReq[async_div], true, true, true, false, true, false, true);
  ...
  for(k=0+Proc.lev_hw; k<32+Proc.lev_hw; k+=1){
    for(j=MAX(Proc.lat_beg, 0)-Proc.lat_beg+Proc.lat_hw; j<MIN(Proc.lat_end+1, 1)-Proc.lat_beg+Proc.lat_hw; j+=1){
      for(i=MAX(Proc.lon_beg, 0)-Proc.lon_beg+Proc.lon_hw; i<MIN(Proc.lon_end+1, 360)-Proc.lon_beg+Proc.lon_hw; i+=1){
        star_state->vor[k][j][i] = ((star_state->tmpsum[k][j][i] / 360) / mesh->area_cell[0][j][0]);
      }
    }
  }
  ...
  UpdateHalo_3d_D(Proc, &star_state->pv[0][0][0], &Proc.FieldReq[async_pv], false, false, true, true, true, true, true);
  ...
  UpdateHalo_3d_D(Proc, &star_state->pv_lon[0][0][0], &Proc.FieldReq[async_pv_lon], false, true, true, true, false, false, true);
  ...
  UpdateHalo_3d_D(Proc, &star_state->pv_lat[0][0][0], &Proc.FieldReq[async_pv_lat], true, false, true, false, true, true, false);
  ...
  for(k=0+Proc.lev_hw; k<32+Proc.lev_hw; k+=1){
    for(j=MAX(Proc.lat_beg, 0)-Proc.lat_beg+Proc.lat_hw; j<MIN(Proc.lat_end+1, 1)-Proc.lat_beg+Proc.lat_hw; j+=1){
      for(i=MAX(Proc.lon_beg, 0)-Proc.lon_beg+Proc.lon_hw; i<MIN(Proc.lon_end+1, 360)-Proc.lon_beg+Proc.lon_hw; i+=1){
        tend1->dmfdlat[k][j][i] = (((star_state->tmpsum[k][j][i] * mesh->le_lat[0][j][0]) / 360) / mesh->area_cell[0][j][0]);
      }
    }
  }
  ...
  UpdateHalo_2d_D(Proc, &tend1->dphs[0][0][0], &Proc.FieldReq[async_dphs], true, true, true, true, true, true);
  ...
  UpdateHalo_3d_D(Proc, &star_state->we_lev[0][0][0], &Proc.FieldReq[async_we_lev], true, true, false, false, true, false, true);
  ...
  UpdateHalo_3d_D(Proc, &advpt->qlx[0][0][0], &Proc.FieldReq[async_qlx], true, true, true, true, true, false, false);
  UpdateHalo_3d_D(Proc, &advpt->dqx[0][0][0], &Proc.FieldReq[async_dqx], true, true, true, true, true, false, false);
  UpdateHalo_3d_D(Proc, &advpt->q6x[0][0][0], &Proc.FieldReq[async_q6x], true, true, true, true, true, false, false);
  UpdateHalo_3d_D(Proc, &advpt->qly[0][0][0], &Proc.FieldReq[async_qly], true, true, true, false, false, false, true);
  UpdateHalo_3d_D(Proc, &advpt->dqy[0][0][0], &Proc.FieldReq[async_dqy], true, true, true, false, false, false, true);
  UpdateHalo_3d_D(Proc, &advpt->q6y[0][0][0], &Proc.FieldReq[async_q6y], true, true, true, false, false, false, true);
  ...
  UpdateHalo_3d_D(Proc, &star_state->ptf_lon[0][0][0], &Proc.FieldReq[async_ptf_lon], false, true, true, true, false, false, false);
  ...
  UpdateHalo_3d_D(Proc, &star_state->ptf_lat[0][0][0], &Proc.FieldReq[async_ptf_lat], true, false, true, false, false, true, false);
  ...
  for(k=0+Proc.lev_hw; k<32+Proc.lev_hw; k+=1){
    for(j=MAX(Proc.lat_beg, 0)-Proc.lat_beg+Proc.lat_hw; j<MIN(Proc.lat_end+1, 1)-Proc.lat_beg+Proc.lat_hw; j+=1){
      for(i=MAX(Proc.lon_beg, 0)-Proc.lon_beg+Proc.lon_hw; i<MIN(Proc.lon_end+1, 360)-Proc.lon_beg+Proc.lon_hw; i+=1){
        ...
        advpt->qy[k][j][i] = (star_state->pt[k][j][i] + ((0.5 * ((((star_state->tmpsum[k][j][i] * mesh->le_lat[0][j][0]) / 360) / mesh->area_cell[0][j][0]) - (advpt->divy[k][j][i] * star_state->pt[k][j][i]))) * dt));
      }
    }
  }
  UpdateHalo_3d_D(Proc, &advpt->qx[0][0][0], &Proc.FieldReq[async_qx], true, true, true, false, false, true, true);
  UpdateHalo_3d_D(Proc, &advpt->qy[0][0][0], &Proc.FieldReq[async_qy], true, true, true, true, true, false, false);
  ...
  for(k=0+Proc.lev_hw; k<32+Proc.lev_hw; k+=1){
    for(j=MAX(Proc.lat_beg, 0)-Proc.lat_beg+Proc.lat_hw; j<MIN(Proc.lat_end+1, 1)-Proc.lat_beg+Proc.lat_hw; j+=1){
      for(i=MAX(Proc.lon_beg, 0)-Proc.lon_beg+Proc.lon_hw; i<MIN(Proc.lon_end+1, 360)-Proc.lon_beg+Proc.lon_hw; i+=1){
        tend1->dptfdlat[k][j][i] = (((star_state->tmpsum[k][j][i] * mesh->le_lat[0][j][0]) / 360) / mesh->area_cell[0][j][0]);
      }
    }
  }
  ...
  UpdateHalo_3d_D(Proc, &star_state->pt[0][0][0], &Proc.FieldReq[async_pt], true, true, true, true, true, true, true);
  ...
  UpdateHalo_3d_D(Proc, &tend1->du[0][0][0], &Proc.FieldReq[async_du], false, true, true, true, true, true, true);
  ...
  UpdateHalo_3d_D(Proc, &tend1->dv[0][0][0], &Proc.FieldReq[async_dv], true, false, true, true, true, true, true);
  ...
  UpdateHalo_3d_D(Proc, &tend1->dpt[0][0][0], &Proc.FieldReq[async_dpt], true, true, true, true, true, true, true);
  ...
  if ((tendPara->phs))
  {
    ...
    UpdateHalo_2d_D(Proc, &new_state->phs[0][0][0], &Proc.FieldReq[async_phs], true, true, true, true, true, true);
    ...
    UpdateHalo_3d_D(Proc, &new_state->ph_exn_lev[0][0][0], &Proc.FieldReq[async_ph_exn_lev], true, true, false, false, true, false, true);
    ...
    UpdateHalo_3d_D(Proc, &new_state->m[0][0][0], &Proc.FieldReq[async_m], true, true, true, false, true, false, true);
  ...
  if ((tendPara->pt))
  {
    ...
    UpdateHalo_3d_D(Proc, &new_state->pt[0][0][0], &Proc.FieldReq[async_pt], true, true, true, true, true, true, true);
  ...
  if (((tendPara->u) && (tendPara->v)))
  {
    ...
    UpdateHalo_3d_D(Proc, &new_state->u_lon[0][0][0], &Proc.FieldReq[async_u_lon], false, true, true, true, true, true, true);
    ...
    UpdateHalo_3d_D(Proc, &new_state->v_lat[0][0][0], &Proc.FieldReq[async_v_lat], true, false, true, true, true, true, true);
  ...
  UpdateHalo_3d_D(Proc, &new_state->gz_lev[0][0][0], &Proc.FieldReq[async_gz_lev], true, true, false, false, true, false, true);
  ...
  UpdateHalo_3d_D(Proc, &tend2->du[0][0][0], &Proc.FieldReq[async_du], false, true, true, true, true, true, true);
  ...
  UpdateHalo_3d_D(Proc, &tend2->dv[0][0][0], &Proc.FieldReq[async_dv], true, false, true, true, true, true, true);
  ...
  if ((tendPara->phs))
  {
    ...
    UpdateHalo_2d_D(Proc, &tend2->dphs[0][0][0], &Proc.FieldReq[async_dphs], true, true, true, true, true, true);
  ...
  if ((tendPara->pt))
  {
    ...
    UpdateHalo_3d_D(Proc, &tend2->dpt[0][0][0], &Proc.FieldReq[async_dpt], true, true, true, true, true, true, true);
...
void damp_run_0(struct HybridStateField* state, struct HybridTendField* tend, struct HybridMeshField* mesh, double dt)
{
  ...
  UpdateHalo_3d_D(Proc, &state->smag_t[0][0][0], &Proc.FieldReq[async_smag_t], true, true, true, false, true, false, true);
  ...
  UpdateHalo_3d_D(Proc, &state->smag_s[0][0][0], &Proc.FieldReq[async_smag_s], false, false, true, true, false, true, false);
  ...
  HaloWait(Proc,&Proc.FieldReq[async_v_lat]);
  for(k=0+Proc.lev_hw; k<32+Proc.lev_hw; k+=1){
    for(j=MAX(Proc.lat_beg, 1)-Proc.lat_beg+Proc.lat_hw; j<MIN(Proc.lat_end+1, 178)-Proc.lat_beg+Proc.lat_hw; j+=1){
      for(i=MAX(Proc.lon_beg, 0)-Proc.lon_beg+Proc.lon_hw; i<MIN(Proc.lon_end+1, 360)-Proc.lon_beg+Proc.lon_hw; i+=1){
...
void adv_run_0(struct HybridStateField* state_old, struct HybridStateField* state_new, struct HybridAdvField* adv, struct HybridAdvPara* advPara, struct HybridMeshField* mesh, double dt)
{
  ...
  if (((advPara->uv_step) == -1))
  {
    ...
    UpdateHalo_3d_D(Proc, &adv->uu[0][0][0], &Proc.FieldReq[async_uu], false, true, true, true, false, false, false);
    ...
    UpdateHalo_3d_D(Proc, &adv->vv[0][0][0], &Proc.FieldReq[async_vv], true, false, true, false, false, true, false);
  ...
  if (((advPara->dynamic) || ((advPara->uv_step) > (advPara->nstep))))
  {
    ...
    // SumCall:state_old->tmpsum.[0, 0, 32, 0, 0, 1, 1, 0, 360]
    ...
    for(k=0+Proc.lev_hw; k<32+Proc.lev_hw; k+=1){
      for(j=MAX(Proc.lat_beg, 0)-Proc.lat_beg+Proc.lat_hw; j<MIN(Proc.lat_end+1, 1)-Proc.lat_beg+Proc.lat_hw; j+=1){
        for(i=MAX(Proc.lon_beg, 0)-Proc.lon_beg+Proc.lon_hw; i<MIN(Proc.lon_end+1, 360)-Proc.lon_beg+Proc.lon_hw; i+=1){
          adv->divy[k][j][i] = (((state_old->tmpsum[k][j][i] * mesh->le_lat[0][j][0]) / 360) / mesh->area_cell[0][j][0]);
        }
      }
    }
  ...
  UpdateHalo_3d_D(Proc, &adv->qlx[0][0][0], &Proc.FieldReq[async_qlx], true, true, true, true, true, false, false);
  UpdateHalo_3d_D(Proc, &adv->dqx[0][0][0], &Proc.FieldReq[async_dqx], true, true, true, true, true, false, false);
  UpdateHalo_3d_D(Proc, &adv->q6x[0][0][0], &Proc.FieldReq[async_q6x], true, true, true, true, true, false, false);
  UpdateHalo_3d_D(Proc, &adv->qly[0][0][0], &Proc.FieldReq[async_qly], true, true, true, false, false, false, true);
  UpdateHalo_3d_D(Proc, &adv->dqy[0][0][0], &Proc.FieldReq[async_dqy], true, true, true, false, false, false, true);
  UpdateHalo_3d_D(Proc, &adv->q6y[0][0][0], &Proc.FieldReq[async_q6y], true, true, true, false, false, false, true);
  ...
  UpdateHalo_3d_D(Proc, &adv->qmf_lon[0][0][0], &Proc.FieldReq[async_qmf_lon], false, true, true, true, false, false, false);
  ...
  UpdateHalo_3d_D(Proc, &adv->qmf_lat[0][0][0], &Proc.FieldReq[async_qmf_lat], true, false, true, false, false, true, false);
  ...
  for(k=0+Proc.lev_hw; k<32+Proc.lev_hw; k+=1){
    for(j=MAX(Proc.lat_beg, 0)-Proc.lat_beg+Proc.lat_hw; j<MIN(Proc.lat_end+1, 1)-Proc.lat_beg+Proc.lat_hw; j+=1){
      for(i=MAX(Proc.lon_beg, 0)-Proc.lon_beg+Proc.lon_hw; i<MIN(Proc.lon_end+1, 360)-Proc.lon_beg+Proc.lon_hw; i+=1){
        ...
        adv->qy[k][j][i] = (state_old->qv[k][j][i] + ((0.5 * ((((state_old->tmpsum[k][j][i] * mesh->le_lat[0][j][0]) / 360) / mesh->area_cell[0][j][0]) - (adv->divy[k][j][i] * state_old->qv[k][j][i]))) * dt));
      }
    }
  }
  UpdateHalo_3d_D(Proc, &adv->qx[0][0][0], &Proc.FieldReq[async_qx], true, true, true, false, false, true, true);
  UpdateHalo_3d_D(Proc, &adv->qy[0][0][0], &Proc.FieldReq[async_qy], true, true, true, true, true, false, false);
  ...
  for(k=0+Proc.lev_hw; k<32+Proc.lev_hw; k+=1){
    for(j=MAX(Proc.lat_beg, 0)-Proc.lat_beg+Proc.lat_hw; j<MIN(Proc.lat_end+1, 180)-Proc.lat_beg+Proc.lat_hw; j+=1){
      for(i=MAX(Proc.lon_beg, 0)-Proc.lon_beg+Proc.lon_hw; i<MIN(Proc.lon_end+1, 360)-Proc.lon_beg+Proc.lon_hw; i+=1){
        state_new->qv[k][j][i] = ((adv->old_m[k][j][i] * state_old->qv[k][j][i]) / state_old->m[k][j][i]);
      }
    }
  }
  ...
  UpdateHalo_3d_D(Proc, &state_new->qv[0][0][0], &Proc.FieldReq[async_qv], true, true, true, true, true, true, true);
...
void operatorPrepareNull_2(struct HybridStateField* state, struct HybridStaticField* staticv, struct HybridAdvField* advpt, struct HybridAdvPara* advPara, struct HybridMeshField* mesh, double dt)
{
  ...
  if (((advPara->dynamic) || ((advPara->uv_step) > (advPara->nstep))))
  {
    ...
    HaloWait(Proc,&Proc.FieldReq[async_vv]);
    for(k=0+Proc.lev_hw; k<32+Proc.lev_hw; k+=1){
      for(j=MAX(Proc.lat_beg, 179)-Proc.lat_beg+Proc.lat_hw; j<MIN(Proc.lat_end+1, 180)-Proc.lat_beg+Proc.lat_hw; j+=1){
        for(i=MAX(Proc.lon_beg, 0)-Proc.lon_beg+Proc.lon_hw; i<MIN(Proc.lon_end+1, 360)-Proc.lon_beg+Proc.lon_hw; i+=1){
    ...
    // SumCall:state->tmpsum.[0, 0, 32, 0, 179, 180, 1, 0, 360]
    tmp_state_tmpsum = allocate_1d_array_D(32);
    for(k=0+Proc.lev_hw; k<32+Proc.lev_hw; k+=1){
      for(j=MAX(Proc.lat_beg, 179)-Proc.lat_beg+Proc.lat_hw; j<MIN(Proc.lat_end+1, 180)-Proc.lat_beg+Proc.lat_hw; j+=1){
        for(i=MAX(Proc.lon_beg, 0)-Proc.lon_beg+Proc.lon_hw; i<MIN(Proc.lon_end+1, 360)-Proc.lon_beg+Proc.lon_hw; i+=1){
    ...
    for(k=0+Proc.lev_hw; k<32+Proc.lev_hw; k+=1){
      for(j=MAX(Proc.lat_beg, 179)-Proc.lat_beg+Proc.lat_hw; j<MIN(Proc.lat_end+1, 180)-Proc.lat_beg+Proc.lat_hw; j+=1){
        for(i=MAX(Proc.lon_beg, 0)-Proc.lon_beg+Proc.lon_hw; i<MIN(Proc.lon_end+1, 360)-Proc.lon_beg+Proc.lon_hw; i+=1){
          advpt->divy[k][j][i] = (((state->tmpsum[k][j][i] * mesh->le_lat[0][(j - 1)][0]) / 360) / mesh->area_cell[0][j][0]);
        }
      }
    }
  ...
  for(k=0+Proc.lev_hw; k<32+Proc.lev_hw; k+=1){
    for(j=MAX(Proc.lat_beg, 178)-Proc.lat_beg+Proc.lat_hw; j<MIN(Proc.lat_end+1, 179)-Proc.lat_beg+Proc.lat_hw; j+=1){
      for(i=MAX(Proc.lon_beg, 0)-Proc.lon_beg+Proc.lon_hw; i<MIN(Proc.lon_end+1, 360)-Proc.lon_beg+Proc.lon_hw; i+=1){
        state->tmpsum[k][j][i] = (state->u_lat[k][j][i] * mesh->le_lat[0][j][0]);
      }
    }
  ...
  // SumCall:state->tmpsum.[0, 0, 32, 0, 178, 179, 1, 0, 360]
  tmp_state_tmpsum = allocate_1d_array_D(32);
  for(k=0+Proc.lev_hw; k<32+Proc.lev_hw; k+=1){
    for(j=MAX(Proc.lat_beg, 178)-Proc.lat_beg+Proc.lat_hw; j<MIN(Proc.lat_end+1, 179)-Proc.lat_beg+Proc.lat_hw; j+=1){
      for(i=MAX(Proc.lon_beg, 0)-Proc.lon_beg+Proc.lon_hw; i<MIN(Proc.lon_end+1, 360)-Proc.lon_beg+Proc.lon_hw; i+=1){
  ...
  for(k=0+Proc.lev_hw; k<32+Proc.lev_hw; k+=1){
    for(j=MAX(Proc.lat_beg, 178)-Proc.lat_beg+Proc.lat_hw; j<MIN(Proc.lat_end+1, 179)-Proc.lat_beg+Proc.lat_hw; j+=1){
      for(i=MAX(Proc.lon_beg, 0)-Proc.lon_beg+Proc.lon_hw; i<MIN(Proc.lon_end+1, 360)-Proc.lon_beg+Proc.lon_hw; i+=1){
        state->vor[k][j][i] = ((state->tmpsum[k][j][i] / 360) / mesh->area_cell[0][(j + 1)][0]);
      }
    }
  }
  ...
  for(k=0+Proc.lev_hw; k<32+Proc.lev_hw; k+=1){
    for(j=MAX(Proc.lat_beg, 179)-Proc.lat_beg+Proc.lat_hw; j<MIN(Proc.lat_end+1, 180)-Proc.lat_beg+Proc.lat_hw; j+=1){
      for(i=MAX(Proc.lon_beg, 0)-Proc.lon_beg+Proc.lon_hw; i<MIN(Proc.lon_end+1, 360)-Proc.lon_beg+Proc.lon_hw; i+=1){
        state->div[k][j][i] = (((state->tmpsum[k][j][i] * mesh->le_lat[0][(j - 1)][0]) / 360) / mesh->area_cell[0][j][0]);
      }
    }
  }
  UpdateHalo_3d_D(Proc, &state->div[0][0][0], &Proc.FieldReq[async_div], true, true, true, false, true, false, true);
...
void stepForwardBackward_2(struct HybridStateField* old_state, struct HybridStateField* star_state, struct HybridStateField* new_state, struct HybridStaticField* staticv, struct HybridTendField* tend1, struct HybridTendField* tend2, struct HybridTendPara* tendPara, struct HybridAdvField* advpt, struct HybridAdvPara* advPara, struct HybridMeshField* mesh, double dt)
{
  ...
  if (((advPara->dynamic) || ((advPara->uv_step) > (advPara->nstep))))
  {
    ...
    // SumCall:star_state->tmpsum.[0, 0, 32, 0, 179, 180, 1, 0, 360]
    tmp_star_state_tmpsum = allocate_1d_array_D(32);
    for(k=0+Proc.lev_hw; k<32+Proc.lev_hw; k+=1){
      for(j=MAX(Proc.lat_beg, 179)-Proc.lat_beg+Proc.lat_hw; j<MIN(Proc.lat_end+1, 180)-Proc.lat_beg+Proc.lat_hw; j+=1){
        for(i=MAX(Proc.lon_beg, 0)-Proc.lon_beg+Proc.lon_hw; i<MIN(Proc.lon_end+1, 360)-Proc.lon_beg+Proc.lon_hw; i+=1){
    ...
    for(k=0+Proc.lev_hw; k<32+Proc.lev_hw; k+=1){
      for(j=MAX(Proc.lat_beg, 179)-Proc.lat_beg+Proc.lat_hw; j<MIN(Proc.lat_end+1, 180)-Proc.lat_beg+Proc.lat_hw; j+=1){
        for(i=MAX(Proc.lon_beg, 0)-Proc.lon_beg+Proc.lon_hw; i<MIN(Proc.lon_end+1, 360)-Proc.lon_beg+Proc.lon_hw; i+=1){
          advpt->divy[k][j][i] = (((star_state->tmpsum[k][j][i] * mesh->le_lat[0][(j - 1)][0]) / 360) / mesh->area_cell[0][j][0]);
        }
      }
    }
  ...
  for(k=0+Proc.lev_hw; k<32+Proc.lev_hw; k+=1){
    for(j=MAX(Proc.lat_beg, 179)-Proc.lat_beg+Proc.lat_hw; j<MIN(Proc.lat_end+1, 180)-Proc.lat_beg+Proc.lat_hw; j+=1){
      for(i=MAX(Proc.lon_beg, 0)-Proc.lon_beg+Proc.lon_hw; i<MIN(Proc.lon_end+1, 360)-Proc.lon_beg+Proc.lon_hw; i+=1){
        star_state->div[k][j][i] = (((star_state->tmpsum[k][j][i] * mesh->le_lat[0][(j - 1)][0]) / 360) / mesh->area_cell[0][j][0]);
      }
    }
  }
  UpdateHalo_3d_D(Proc, &star_state->div[0][0][0], &Proc.FieldReq[async_div], true, true, true, false, true, false, true);
  ...
  // SumCall:star_state->tmpsum.[0, 0, 32, 0, 178, 179, 1, 0, 360]
  tmp_star_state_tmpsum = allocate_1d_array_D(32);
  for(k=0+Proc.lev_hw; k<32+Proc.lev_hw; k+=1){
    for(j=MAX(Proc.lat_beg, 178)-Proc.lat_beg+Proc.lat_hw; j<MIN(Proc.lat_end+1, 179)-Proc.lat_beg+Proc.lat_hw; j+=1){
      for(i=MAX(Proc.lon_beg, 0)-Proc.lon_beg+Proc.lon_hw; i<MIN(Proc.lon_end+1, 360)-Proc.lon_beg+Proc.lon_hw; i+=1){
  ...
  for(k=0+Proc.lev_hw; k<32+Proc.lev_hw; k+=1){
    for(j=MAX(Proc.lat_beg, 178)-Proc.lat_beg+Proc.lat_hw; j<MIN(Proc.lat_end+1, 179)-Proc.lat_beg+Proc.lat_hw; j+=1){
      for(i=MAX(Proc.lon_beg, 0)-Proc.lon_beg+Proc.lon_hw; i<MIN(Proc.lon_end+1, 360)-Proc.lon_beg+Proc.lon_hw; i+=1){
        star_state->vor[k][j][i] = ((star_state->tmpsum[k][j][i] / 360) / mesh->area_cell[0][(j + 1)][0]);
      }
    }
  }
  ...
  for(k=0+Proc.lev_hw; k<32+Proc.lev_hw; k+=1){
    for(j=MAX(Proc.lat_beg, 179)-Proc.lat_beg+Proc.lat_hw; j<MIN(Proc.lat_end+1, 180)-Proc.lat_beg+Proc.lat_hw; j+=1){
      for(i=MAX(Proc.lon_beg, 0)-Proc.lon_beg+Proc.lon_hw; i<MIN(Proc.lon_end+1, 360)-Proc.lon_beg+Proc.lon_hw; i+=1){
        tend1->dmfdlat[k][j][i] = (((star_state->tmpsum[k][j][i] * mesh->le_lat[0][(j - 1)][0]) / 360) / mesh->area_cell[0][j][0]);
      }
    }
  }
  ...
  for(k=0+Proc.lev_hw; k<32+Proc.lev_hw; k+=1){
    for(j=MAX(Proc.lat_beg, 179)-Proc.lat_beg+Proc.lat_hw; j<MIN(Proc.lat_end+1, 180)-Proc.lat_beg+Proc.lat_hw; j+=1){
      for(i=MAX(Proc.lon_beg, 0)-Proc.lon_beg+Proc.lon_hw; i<MIN(Proc.lon_end+1, 360)-Proc.lon_beg+Proc.lon_hw; i+=1){
        advpt->qx[k][j][i] = star_state->pt[k][j][i];
        advpt->qy[k][j][i] = (star_state->pt[k][j][i] + ((0.5 * ((((star_state->tmpsum[k][j][i] * mesh->le_lat[0][(j - 1)][0]) / 360) / mesh->area_cell[0][j][0]) - (advpt->divy[k][j][i] * star_state->pt[k][j][i]))) * dt));
      }
    }
  }
  UpdateHalo_3d_D(Proc, &advpt->qx[0][0][0], &Proc.FieldReq[async_qx], true, true, true, false, false, true, true);
  ...
  for(k=0+Proc.lev_hw; k<32+Proc.lev_hw; k+=1){
    for(j=MAX(Proc.lat_beg, 179)-Proc.lat_beg+Proc.lat_hw; j<MIN(Proc.lat_end+1, 180)-Proc.lat_beg+Proc.lat_hw; j+=1){
      for(i=MAX(Proc.lon_beg, 0)-Proc.lon_beg+Proc.lon_hw; i<MIN(Proc.lon_end+1, 360)-Proc.lon_beg+Proc.lon_hw; i+=1){
        tend1->dptfdlat[k][j][i] = (((star_state->tmpsum[k][j][i] * mesh->le_lat[0][(j - 1)][0]) / 360) / mesh->area_cell[0][j][0]);
      }
    }
  }
...
void adv_run_2(struct HybridStateField* state_old, struct HybridStateField* state_new, struct HybridAdvField* adv, struct HybridAdvPara* advPara, struct HybridMeshField* mesh, double dt)
{
  ...
  if (((advPara->dynamic) || ((advPara->uv_step) > (advPara->nstep))))
  {
    ...
    // SumCall:state_old->tmpsum.[0, 0, 32, 0, 179, 180, 1, 0, 360]
    tmp_state_old_tmpsum = allocate_1d_array_D(32);
    for(k=0+Proc.lev_hw; k<32+Proc.lev_hw; k+=1){
      for(j=MAX(Proc.lat_beg, 179)-Proc.lat_beg+Proc.lat_hw; j<MIN(Proc.lat_end+1, 180)-Proc.lat_beg+Proc.lat_hw; j+=1){
        for(i=MAX(Proc.lon_beg, 0)-Proc.lon_beg+Proc.lon_hw; i<MIN(Proc.lon_end+1, 360)-Proc.lon_beg+Proc.lon_hw; i+=1){
    ...
    for(k=0+Proc.lev_hw; k<32+Proc.lev_hw; k+=1){
      for(j=MAX(Proc.lat_beg, 179)-Proc.lat_beg+Proc.lat_hw; j<MIN(Proc.lat_end+1, 180)-Proc.lat_beg+Proc.lat_hw; j+=1){
        for(i=MAX(Proc.lon_beg, 0)-Proc.lon_beg+Proc.lon_hw; i<MIN(Proc.lon_end+1, 360)-Proc.lon_beg+Proc.lon_hw; i+=1){
          adv->divy[k][j][i] = (((state_old->tmpsum[k][j][i] * mesh->le_lat[0][(j - 1)][0]) / 360) / mesh->area_cell[0][j][0]);
        }
      }
    }
  ...
  for(k=0+Proc.lev_hw; k<32+Proc.lev_hw; k+=1){
    for(j=MAX(Proc.lat_beg, 179)-Proc.lat_beg+Proc.lat_hw; j<MIN(Proc.lat_end+1, 180)-Proc.lat_beg+Proc.lat_hw; j+=1){
      for(i=MAX(Proc.lon_beg, 0)-Proc.lon_beg+Proc.lon_hw; i<MIN(Proc.lon_end+1, 360)-Proc.lon_beg+Proc.lon_hw; i+=1){
        adv->qx[k][j][i] = state_old->qv[k][j][i];
        adv->qy[k][j][i] = (state_old->qv[k][j][i] + ((0.5 * ((((state_old->tmpsum[k][j][i] * mesh->le_lat[0][(j - 1)][0]) / 360) / mesh->area_cell[0][j][0]) - (adv->divy[k][j][i] * state_old->qv[k][j][i]))) * dt));
      }
    }
  }
  UpdateHalo_3d_D(Proc, &adv->qx[0][0][0], &Proc.FieldReq[async_qx], true, true, true, false, false, true, true);
...
int main(int argc, char **argv)
{
  ...
  //Proc Ngb Init
  if (Proc.lat_beg >= 0 && Proc.lat_end <= 2){
    ProcInit_LonLat_Ngb(&Proc,MPI_COMM_WORLD,ProcLatNum,ProcLon,ProcLat,nlon,nlat,nlev,67, 2, 2, 2, DTYPE_DOUBLE, 108);
  }
  if (Proc.lat_beg >= 3 && Proc.lat_end <= 5){
    ProcInit_LonLat_Ngb(&Proc,MPI_COMM_WORLD,ProcLatNum,ProcLon,ProcLat,nlon,nlat,nlev,13, 2, 2, 2, DTYPE_DOUBLE, 108);
  }
  if (Proc.lat_beg >= 6 && Proc.lat_end <= 8){
    ProcInit_LonLat_Ngb(&Proc,MPI_COMM_WORLD,ProcLatNum,ProcLon,ProcLat,nlon,nlat,nlev,7, 2, 2, 2, DTYPE_DOUBLE, 108);
  }
  if (Proc.lat_beg >= 9 && Proc.lat_end <= 11){
    ProcInit_LonLat_Ngb(&Proc,MPI_COMM_WORLD,ProcLatNum,ProcLon,ProcLat,nlon,nlat,nlev,5, 2, 2, 2, DTYPE_DOUBLE, 108);
  }
  if (Proc.lat_beg >= 12 && Proc.lat_end <= 14){
    ProcInit_LonLat_Ngb(&Proc,MPI_COMM_WORLD,ProcLatNum,ProcLon,ProcLat,nlon,nlat,nlev,4, 2, 2, 2, DTYPE_DOUBLE, 108);
  }
  if (Proc.lat_beg >= 15 && Proc.lat_end <= 26){
    ProcInit_LonLat_Ngb(&Proc,MPI_COMM_WORLD,ProcLatNum,ProcLon,ProcLat,nlon,nlat,nlev,3, 2, 2, 2, DTYPE_DOUBLE, 108);
  }
  if (Proc.lat_beg >= 27 && Proc.lat_end <= 152){
    ProcInit_LonLat_Ngb(&Proc,MPI_COMM_WORLD,ProcLatNum,ProcLon,ProcLat,nlon,nlat,nlev,2, 2, 2, 2, DTYPE_DOUBLE, 108);
  }
  if (Proc.lat_beg >= 153 && Proc.lat_end <= 161){
    ProcInit_LonLat_Ngb(&Proc,MPI_COMM_WORLD,ProcLatNum,ProcLon,ProcLat,nlon,nlat,nlev,3, 2, 2, 2, DTYPE_DOUBLE, 108);
  }
  if (Proc.lat_beg >= 162 && Proc.lat_end <= 167){
    ProcInit_LonLat_Ngb(&Proc,MPI_COMM_WORLD,ProcLatNum,ProcLon,ProcLat,nlon,nlat,nlev,4, 2, 2, 2, DTYPE_DOUBLE, 108);
  }
  if (Proc.lat_beg >= 168 && Proc.lat_end <= 170){
    ProcInit_LonLat_Ngb(&Proc,MPI_COMM_WORLD,ProcLatNum,ProcLon,ProcLat,nlon,nlat,nlev,5, 2, 2, 2, DTYPE_DOUBLE, 108);
  }
  if (Proc.lat_beg >= 171 && Proc.lat_end <= 173){
    ProcInit_LonLat_Ngb(&Proc,MPI_COMM_WORLD,ProcLatNum,ProcLon,ProcLat,nlon,nlat,nlev,8, 2, 2, 2, DTYPE_DOUBLE, 108);
  }
  if (Proc.lat_beg >= 174 && Proc.lat_end <= 176){
    ProcInit_LonLat_Ngb(&Proc,MPI_COMM_WORLD,ProcLatNum,ProcLon,ProcLat,nlon,nlat,nlev,16, 2, 2, 2, DTYPE_DOUBLE, 108);
  }
  if (Proc.lat_beg >= 177 && Proc.lat_end <= 179){
    ProcInit_LonLat_Ngb(&Proc,MPI_COMM_WORLD,ProcLatNum,ProcLon,ProcLat,nlon,nlat,nlev,67, 2, 2, 2, DTYPE_DOUBLE, 108);
  }
  ...
  if (Proc.lat_beg >= 0 && Proc.lat_end <= 2)
  {
    ...
    timeOperatorInit_0(state, staticv, adv, &advptPara, mesh, 240);
    tottime_beg = MPI_Wtime();
    for (int t = 0; t < 30; t += 1){
      wrk3d_0(state, staticv, tend, &tendPara, adv, &advptPara, &advmPara, mesh, 240, 120.0, 80.0);
    }
    tottime_end = MPI_Wtime();
  }
  
  if (Proc.lat_beg >= 3 && Proc.lat_end <= 176)
  {
    MeshInit_1(global_mesh, mesh);
    timeOperatorInit_1(state, staticv, adv, &advptPara, mesh, 240);
    tottime_beg = MPI_Wtime();
    for (int t = 0; t < 30; t += 1){
      wrk3d_1(state, staticv, tend, &tendPara, adv, &advptPara, &advmPara, mesh, 240, 120.0, 80.0);
    }
    tottime_end = MPI_Wtime();
  }
  ...
  if (Proc.lat_beg >= 177 && Proc.lat_end <= 179)
  {
    ...
    timeOperatorInit_2(state, staticv, adv, &advptPara, mesh, 240);
    tottime_beg = MPI_Wtime();
    for (int t = 0; t < 30; t += 1){
      wrk3d_2(state, staticv, tend, &tendPara, adv, &advptPara, &advmPara, mesh, 240, 120.0, 80.0);
    }
    tottime_end = MPI_Wtime();
  }
