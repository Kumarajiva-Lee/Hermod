...
void ProcInit_LonLat_Ngb(ProcType *Proc, MPI_Comm comm,int* ProcLatNum ,int* ProcLon ,int* ProcLat ,int NumLon, int NumLat, int NumLev, int HaloSizeLon, int HaloSizeLat, int HaloSizeLev, int CornerHalo, int dtype, int FieldNum){
...
//CubedShpere
//pos = 0 中心 pos = 1 边角
void NgbTypeInitCS(NgbType *Ngb,int id, int type, int orient, int pos, int rotate, int tag, int lonbeg, int lonend, int latbeg, int latend){
    Ngb->id = id;
    Ngb->type = type;
    Ngb->orient = orient;
    Ngb->pos = pos;
...
void HaloTypeInitCS(HaloType *Halo, ProcType *Proc, int HaloType, int orient, int dtype, int host_id, int ngb_id, int tag, \
    ...
    for (k = 0; k < 2; k ++){
        ...
        for (j = 0; j < 2; j++)
            for (i = 0; i < 2; i++){
                printf("Check size %d %d %d %d %d %d %d %d\n",j,i,array_size[j][i][2],subarray_start[j][i][2],subarray_size[j][i][2],array_size[j][i][1],subarray_start[j][i][1],subarray_size[j][i][1]);
                if (dtype == DTYPE_INT){
                    MPI_Type_create_subarray(3, array_size[j][i], subarray_size[j][i], subarray_start[j][i], MPI_ORDER_C, MPI_INT, &(Halo->mpi_type_3d[k][j][i]));
                }
                else if (dtype == DTYPE_SINGLE){
...
void ProcInit_CubedSphere_Ngb(ProcType *Proc, MPI_Comm comm,int size, int id, int ProcX ,int ProcY ,int NumCell, int NumLev, int HaloX, int HaloY, int HalosizeLev, int HaloP, int dtype, int FieldNum){
    ...
    if (! Proc->at_west){
        //不在边界
        wid = id - ProcY;
        NgbTypeInitCS(&Proc->NgbRecvW[Proc->NgbWR++], wid, recv_type, WEST, 0, 0, id+WEST, 0, HaloX-1, HaloY, HaloY+ny-1);
        NgbTypeInitCS(&Proc->NgbSendE[Proc->NgbES++], wid, send_type, EAST, 0, 0, wid+EAST, HaloX, 2*HaloX-1, HaloY, HaloY+ny-1);
    }
    else{
        //在边,分奇偶
        if (Proc->panel & 1){
            ...
            NgbTypeInitCS(&Proc->NgbRecvW[Proc->NgbWR++], wid, recv_type, WEST, 1, 1, id+WEST, HaloP, HaloP+ny-1, 0, HaloX-1);
            NgbTypeInitCS(&Proc->NgbSendN[Proc->NgbNS++], wid, send_type, NORTH, 1, 0, wid+NORTH, HaloX, 2*HaloX-1, HaloY, HaloY+ny-1);

            if (Proc->at_south){
                //在左下角               
                pid = wid - ProcY;
                NgbTypeInitCS(&Proc->NgbRecvW[Proc->NgbWR++], pid, recv_type, WEST, 1, 1, id+WEST+NORTH, 0, HaloP-1, 0, HaloX-1);
                NgbTypeInitCS(&Proc->NgbSendN[Proc->NgbNS++], pid, send_type, NORTH, 1, 0, pid+NORTH+EAST, HaloX, 2*HaloX-1, HaloY+ny-HaloP, HaloY+ny-1);
            }
            else if (Proc->at_north){
                //在左上角
                pid = wid + ProcY;
                NgbTypeInitCS(&Proc->NgbRecvW[Proc->NgbWR++], pid, recv_type, WEST, 1, 1, id+WEST+SOUTH, HaloP+ny, 2*HaloP+ny-1, 0, HaloX-1);
                NgbTypeInitCS(&Proc->NgbSendN[Proc->NgbNS++], pid, send_type, NORTH, 1, 0, pid+NORTH+WEST, HaloX, 2*HaloX-1, HaloY, HaloY+HaloP-1);
            }
            else{
                //普通边
                pid = wid - ProcY;
            ...
            NgbTypeInitCS(&Proc->NgbRecvW[Proc->NgbWR++], wid, recv_type, WEST, 1, 0, id+WEST, 0, HaloX-1, HaloP, HaloP+ny-1);
            NgbTypeInitCS(&Proc->NgbSendE[Proc->NgbES++], wid, send_type, EAST, 1, 0, wid+EAST, HaloX, 2*HaloX-1, HaloY, HaloY+ny-1);

            if (Proc->at_south){
                //在左下角
                pid = wid + 1;
                NgbTypeInitCS(&Proc->NgbRecvW[Proc->NgbWR++], pid, recv_type, WEST, 1, 0, id+WEST+NORTH, 0, HaloX-1, HaloP+ny, 2*HaloP+ny-1);
                NgbTypeInitCS(&Proc->NgbSendE[Proc->NgbES++], pid, send_type, EAST, 1, 0, pid+EAST+SOUTH, HaloX, 2*HaloX-1, HaloY+ny-HaloP, HaloY+ny-1);
            }
            else if (Proc->at_north){
                //在左上角
                pid = wid - 1;
                NgbTypeInitCS(&Proc->NgbRecvW[Proc->NgbWR++], pid, recv_type, WEST, 1, 0, id+WEST+SOUTH, 0, HaloX-1, 0, HaloP-1);
                NgbTypeInitCS(&Proc->NgbSendE[Proc->NgbES++], pid, send_type, EAST, 1, 0, pid+EAST+NORTH, HaloX, 2*HaloX-1, HaloY, HaloY+HaloP-1);
            }
            else{
                //普通边
                pid = wid + 1;
    ...
    if (! Proc->at_east){
        //不在边界
        eid = id + ProcY;
        NgbTypeInitCS(&Proc->NgbRecvE[Proc->NgbER++], eid, recv_type, EAST, 0, 0, id+EAST, HaloX+nx, 2*HaloX+nx-1, HaloY, HaloY+ny-1);
        NgbTypeInitCS(&Proc->NgbSendW[Proc->NgbWS++], eid, send_type, WEST, 0, 0, eid+WEST, nx, nx+HaloX-1, HaloY, HaloY+ny-1);
    }
    else{
        //在边,分奇偶
        if (Proc->panel & 1){
            ...
            NgbTypeInitCS(&Proc->NgbRecvE[Proc->NgbER++], eid, recv_type, EAST, 1, 0, id+EAST, 0, HaloX-1, HaloP, HaloP+ny-1);
            NgbTypeInitCS(&Proc->NgbSendW[Proc->NgbWS++], eid, send_type, WEST, 1, 0, eid+WEST, nx, nx+HaloX-1, HaloY, HaloY+ny-1);

            if (Proc->at_south){
                //右下角
                pid = eid + 1;
                NgbTypeInitCS(&Proc->NgbRecvE[Proc->NgbER++], pid, recv_type, EAST, 1, 0, id+EAST+NORTH, 0, HaloX-1, HaloP+ny, 2*HaloP+ny-1);
                NgbTypeInitCS(&Proc->NgbSendW[Proc->NgbWS++], pid, send_type, WEST, 1, 0, pid+WEST+SOUTH, nx, nx+HaloX-1, HaloY+ny-HaloP, HaloY+ny-1);
            }
            else if (Proc->at_north){
                //右上角
                pid = eid - 1;
                NgbTypeInitCS(&Proc->NgbRecvE[Proc->NgbER++], pid, recv_type, EAST, 1, 0, id+EAST+SOUTH, 0, HaloX-1, 0, HaloP-1);
                NgbTypeInitCS(&Proc->NgbSendW[Proc->NgbWS++], pid ,send_type, WEST, 1, 0, pid+WEST+NORTH, nx, nx+HaloX-1, HaloY, HaloY+HaloP-1);
            }
            else{
                //普通边
                pid = eid + 1;
            ...
            NgbTypeInitCS(&Proc->NgbRecvE[Proc->NgbER++], eid, recv_type, EAST, 1, 1, id+EAST, HaloP, HaloP+ny-1, 0, HaloX-1);
            NgbTypeInitCS(&Proc->NgbSendS[Proc->NgbSS++], eid, send_type, SOUTH, 1, 0, eid+SOUTH, nx, nx+HaloX-1, HaloY, HaloY+ny-1);

            if (Proc->at_south){
                //右下角
                pid = eid - ProcY;
                NgbTypeInitCS(&Proc->NgbRecvE[Proc->NgbER++], pid, recv_type, EAST, 1, 1, id+EAST+NORTH, 0, HaloP-1, 0, HaloX-1);
                NgbTypeInitCS(&Proc->NgbSendS[Proc->NgbSS++], pid, send_type, SOUTH, 1, 0, pid+SOUTH+EAST,nx, nx+HaloX-1, HaloY+ny-HaloP, HaloY+ny-1);

            }
            else if (Proc->at_north){
                //右上角
                pid = eid + ProcY;
                NgbTypeInitCS(&Proc->NgbRecvE[Proc->NgbER++], pid, recv_type, EAST, 1, 1, id+EAST+SOUTH, HaloP+ny, 2*HaloP+ny-1, 0, HaloX-1);
                NgbTypeInitCS(&Proc->NgbSendS[Proc->NgbSS++], pid, send_type, SOUTH, 1, 0, pid+SOUTH+WEST, nx, nx+HaloX-1, HaloY, HaloY+HaloP-1);
    ...
    if (!Proc->at_south){
        ...
        if (!Proc->at_west){
            pid = sid - ProcY;
            NgbTypeInitCS(&Proc->NgbRecvS[Proc->NgbSR++], pid, recv_type, SOUTH, 0, 0, id+SOUTH+WEST, 0, HaloX-1, 0, HaloY-1);
            NgbTypeInitCS(&Proc->NgbSendN[Proc->NgbNS++], pid, send_type, NORTH, 0, 0, pid+NORTH+EAST, HaloX, HaloX+HaloX-1, HaloY, 2*HaloY-1);
        }
        //正南
        pid = sid;
        NgbTypeInitCS(&Proc->NgbRecvS[Proc->NgbSR++], pid, recv_type, SOUTH, 0, 0, id+SOUTH, HaloX, HaloX+nx-1, 0, HaloY-1);  
        NgbTypeInitCS(&Proc->NgbSendN[Proc->NgbNS++], pid, send_type, NORTH, 0, 0, pid+NORTH, HaloX, HaloX+nx-1, HaloY, 2*HaloY-1);
        //东南
        if (!Proc->at_east){
            pid = sid + ProcY;
            NgbTypeInitCS(&Proc->NgbRecvS[Proc->NgbSR++], pid, recv_type, SOUTH, 0, 0, id+SOUTH+EAST, HaloX+nx, 2*HaloX+nx-1, 0, HaloY-1);
            NgbTypeInitCS(&Proc->NgbSendN[Proc->NgbNS++], pid, send_type, NORTH, 0, 0, pid+NORTH+WEST, nx, nx+HaloX-1, HaloY, 2*HaloY-1);
        }
    }
    else{
        //在边,分奇偶
        if (Proc->panel & 1){
            ...
            NgbTypeInitCS(&Proc->NgbRecvS[Proc->NgbSR++], sid, recv_type, SOUTH, 1, 0, id+SOUTH, HaloP, HaloP+nx-1, 0, HaloY-1);
            NgbTypeInitCS(&Proc->NgbSendN[Proc->NgbNS++], sid, send_type, NORTH, 1, 0, sid+NORTH, HaloX, HaloX+nx-1, HaloY, 2*HaloY-1);

            if (Proc->at_west){
                //左下角
                pid = sid + ProcY;
                NgbTypeInitCS(&Proc->NgbRecvS[Proc->NgbSR++], pid, recv_type, SOUTH, 1, 0, id+SOUTH+EAST, HaloP+nx, 2*HaloP+nx-1, 0, HaloY-1);
                NgbTypeInitCS(&Proc->NgbSendN[Proc->NgbNS++], pid, send_type, NORTH, 1, 0, pid+NORTH+WEST, nx, nx+HaloX-1, HaloY, 2*HaloY-1);

            }
            else if (Proc->at_east){
                //右下角
                pid = sid - ProcY;
                NgbTypeInitCS(&Proc->NgbRecvS[Proc->NgbSR++], pid, recv_type, SOUTH, 1, 0, id+SOUTH+WEST, 0, HaloP-1, 0, HaloY-1);
                NgbTypeInitCS(&Proc->NgbSendN[Proc->NgbNS++], pid, send_type, NORTH, 1, 0, pid+NORTH+EAST,HaloX, HaloX+HaloX-1, HaloY, 2*HaloY-1);
            }
            else{
                //普通边
                pid = sid + ProcY;
            ...
            NgbTypeInitCS(&Proc->NgbRecvS[Proc->NgbSR++], sid, recv_type, SOUTH, 1, 1, id+SOUTH, 0, HaloX-1, HaloP, HaloP+nx-1);
            NgbTypeInitCS(&Proc->NgbSendE[Proc->NgbES++], sid, send_type, EAST, 1, 0, sid+EAST, HaloX, HaloX+nx-1, HaloY, 2*HaloY-1);

            if (Proc->at_west){
                //左下角
                pid = sid - 1;
                NgbTypeInitCS(&Proc->NgbRecvS[Proc->NgbSR++], pid, recv_type, SOUTH, 1, 1, id+SOUTH+EAST, 0, HaloX-1, 0, HaloP-1);
                NgbTypeInitCS(&Proc->NgbSendE[Proc->NgbES++], pid, send_type, EAST, 1, 0, pid+EAST+NORTH, nx, nx+HaloX-1, HaloY, 2*HaloY-1);
                
            }
            else if (Proc->at_east){
                //右下角
                pid = sid + 1;
                NgbTypeInitCS(&Proc->NgbRecvS[Proc->NgbSR++], pid, recv_type, SOUTH, 1, 1, id+SOUTH+WEST,0, HaloX-1, HaloP+nx, 2*HaloP+nx-1);
                NgbTypeInitCS(&Proc->NgbSendE[Proc->NgbES++], pid, send_type, EAST, 1, 0, pid+EAST+SOUTH, HaloX, HaloX+HaloX-1, HaloY, 2*HaloY-1);
            }
            else{
                //普通边
                pid = sid - 1;
    ...
    if (!Proc->at_north){
        ...
        if (!Proc->at_west){
            pid = nid - ProcY;
            NgbTypeInitCS(&Proc->NgbRecvN[Proc->NgbNR++], pid, recv_type, NORTH, 0, 0, id+NORTH+WEST, 0, HaloX-1, HaloY+ny, 2*HaloY+ny-1);
            NgbTypeInitCS(&Proc->NgbSendS[Proc->NgbSS++], pid, send_type, SOUTH, 0, 0, pid+SOUTH+EAST, HaloX, HaloX+HaloX-1, ny, ny+HaloY-1);
        }
        //正北
        pid = nid;
        NgbTypeInitCS(&Proc->NgbRecvN[Proc->NgbNR++], pid, recv_type, NORTH, 0, 0, id+NORTH, HaloX, HaloX+nx-1, HaloY+ny, 2*HaloY+ny-1);
        NgbTypeInitCS(&Proc->NgbSendS[Proc->NgbSS++], pid, send_type, SOUTH, 0, 0, pid+SOUTH, HaloX, HaloX+nx-1, ny, ny+HaloY-1);
        //东北
        if (!Proc->at_east){
            pid = nid + ProcY;
            NgbTypeInitCS(&Proc->NgbRecvN[Proc->NgbNR++], pid, recv_type, NORTH, 0, 0, id+NORTH+EAST, HaloX+nx, 2*HaloX+nx-1, HaloY+ny, 2*HaloY+ny-1); 
            NgbTypeInitCS(&Proc->NgbSendS[Proc->NgbSS++], pid, send_type, SOUTH, 0, 0, pid+SOUTH+WEST, nx, HaloX+nx-1, ny, ny+HaloY-1);
        } 
    }
    else{
        //在边,分奇偶
        if (Proc->panel & 1){
            ...
            NgbTypeInitCS(&Proc->NgbRecvN[Proc->NgbNR++], nid, recv_type, NORTH, 1, 1, id+NORTH, 0, HaloY-1, HaloP, HaloP+nx-1);
            NgbTypeInitCS(&Proc->NgbSendW[Proc->NgbWS++], nid, send_type, WEST, 1, 0, nid+WEST, HaloX, HaloX+nx-1, ny, ny+HaloY-1);

            if (Proc->at_west){
                //左上角
                pid = nid - 1;
                NgbTypeInitCS(&Proc->NgbRecvN[Proc->NgbNR++], pid, recv_type, NORTH, 1, 1, id+NORTH+EAST, 0, HaloY-1, 0, HaloP-1);
                NgbTypeInitCS(&Proc->NgbSendW[Proc->NgbWS++], pid, send_type, WEST, 1, 0, pid+WEST+NORTH, nx, HaloX+nx-1, ny, ny+HaloY-1);
            }
            else if (Proc->at_east){
                //右上角
                pid = nid + 1;
                NgbTypeInitCS(&Proc->NgbRecvN[Proc->NgbNR++], pid, recv_type, NORTH, 1, 1, id+NORTH+WEST, 0, HaloY-1, HaloP+nx, 2*HaloP+nx-1);
                NgbTypeInitCS(&Proc->NgbSendW[Proc->NgbWS++], pid, send_type, WEST, 1, 0, pid+WEST+SOUTH, HaloX, HaloX+HaloX-1, ny, ny+HaloY-1);
            }
            else{
                //普通边
                pid = nid - 1;
            ...
            NgbTypeInitCS(&Proc->NgbRecvN[Proc->NgbNR++], nid, recv_type, NORTH, 1, 0, id+NORTH, HaloP, HaloP+nx-1, 0, HaloY-1);
            NgbTypeInitCS(&Proc->NgbSendS[Proc->NgbSS++], nid, send_type, SOUTH, 1, 0, nid+SOUTH, HaloX, HaloX+nx-1, ny, ny+HaloY-1);

            if (Proc->at_west){
                //左上角
                pid = nid + ProcY;
                NgbTypeInitCS(&Proc->NgbRecvN[Proc->NgbNR++], pid, recv_type, NORTH, 1, 0, id+NORTH+EAST, HaloP+nx, 2*HaloP+nx-1, 0, HaloY-1);
                NgbTypeInitCS(&Proc->NgbSendS[Proc->NgbSS++], pid, send_type, SOUTH, 1, 0, pid+SOUTH+WEST, nx, HaloX+nx-1, ny, ny+HaloY-1);
            }
            else if (Proc->at_east){
                //右上角
                pid = nid - ProcY;
                NgbTypeInitCS(&Proc->NgbRecvN[Proc->NgbNR++], pid, recv_type, NORTH, 1, 0, id+NORTH+WEST, 0, HaloP-1, 0, HaloY-1);
                NgbTypeInitCS(&Proc->NgbSendS[Proc->NgbSS++], pid, send_type, SOUTH, 1, 0, pid+SOUTH+EAST, HaloX, HaloX+HaloX-1, ny, ny+HaloY-1);
            }
            else{
                //普通边
                pid = nid + ProcY;
